...
def upper_camel_to_snake(variable: str):
    ...
    suffix = re.sub('([A-Z])',
                    lambda m: '_' + m.group(1).lower(), variable[1:])
...
class CodeGenerator:
    ...
    def __init__(self):
        ...
        if CodeGenerator.methods_by_type is None:
        ...
        assert CodeGenerator.cpptype_by_python_type is None
    ...
    def cpp_var_name(json_field: str):
        assert (lower_camel_to_snake(
                snake_to_lower_camel(json_field)) == json_field), (
                    'conversation is not bidirectional')
    ...
    def json_var_name(variable: str):
        assert (snake_to_lower_camel(
                lower_camel_to_snake(variable)) == variable), (
                    'conversation is not bidirectional')
    ...
    def cpp_class_name(json_field: str):
        assert (upper_camel_to_snake(
                snake_to_upper_camel(json_field)) == json_field), (
                    'conversation is not bidirectional')
    ...
    def array_parser_func(
        self, indent: str, array_type: str,
        array_name: str, object_instance: str
    ):
        assert (array_type.find('std::array') != -1), (
                'Expect `std::array<T, size>` string')
        array_parser = ''
        ...
        array_parser += '{}{{\n'.format(indent)
        array_parser += ('{}  const auto& values = json["{}"]'
        ...
        if method is None:
            ...
            array_parser += '{}    FromJson(values[i], {}.{}[i]);\n'.format(
                indent, object_instance, array_name)
        else:
            # parse one of basic type
            # (key in `CodeGenerator.methods_by_type` dictionary)
            array_parser += '{}    {}.{}[i] = values[i].{};\n'.format(
                indent, object_instance, array_name, method)
        array_parser += '{}  }}\n'.format(indent)
        array_parser += '{}}}\n'.format(indent)
    ...
    # `struct` is a cpp struct definition with
    # all it's data members as dictionary
    def dump_json_parser_func(self, class_name: str, struct: list[tuple]):
        ...
        self.footer += ('inline void FromJson(const rapidjson::Value& json'
                        ', {}& {}) {{\n').format(class_name, obj_instance)
        ...
        for typename, variable in struct:
            if typename in self.used:
                ...
                self.footer += (self.array_parser_func(
                                '  ', typename, variable, obj_instance))
            else:
                self.footer += '  {}.{} = json["{}"].{};\n'.format(
                    obj_instance,
                    variable,
        ...
        for key in json_value:
            if isinstance(json_value[key], dict):
                ...
                if isinstance(json_value[key][0], dict):
                    ...
                    # I expect they have same set of keys
                    # while values can differ
                    # TODO: throw exception if the expectations failed

                    # treat array of objects as struct with array data member:
                    # `key` becomes a struct whereas the array
                    # becomes a data member
                    self.structs.append([])
                    self.scopes.append(CodeGenerator.cpp_class_name(key))
                    # generate struct for one of those fields (take 0th)
                    self.generate(json_value[key][0], ostream)
                    value_type = self.choose_class_name()
                    if value_type is None:
                        raise RuntimeError('all class names '
                                           'are already reserved')

                    self.scopes.pop()
                    struct = self.structs.pop()
                    assert len(self.structs) > 0, ('Array must have'
                                                   'an owner (outer struct)')

                    # add array as data member of outer-struct
                    self.structs[-1] += [
                        ('std::array<{}, {}>'.format(value_type, array_size),
                         CodeGenerator.cpp_var_name(key))]
                    CodeGenerator.dump_class(value_type, struct, ostream)
                    self.dump_json_parser_func(value_type, struct)

                else:
                    value_type = type(json_value[key][0]).__name__
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate a c++ '
                                                 'file from the json schema')
    parser.add_argument('-i', '--input', help='path to json schema')
    parser.add_argument('-o', '--output', help='c++ file which will be '
    ...
    template = (
        ...
        '// Date: {}')
    include_guard = '__JSON_AUTOGENERATED_CLASSES_{}__'.format(
        UNIQUE_ID.replace('-', '_'))
    includes = (
        '#include <string>\n' +
        '#include <array>\n\n' +
    ...
    with open(output, 'w') as ostream:
        header = template.format(datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
